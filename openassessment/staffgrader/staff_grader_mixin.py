...
from submissions.errors import SubmissionError, SubmissionInternalError, SubmissionNotFoundError, SubmissionRequestError
...
from openassessment.staffgrader.serializers import (AssessmentSerializer, MissingContextException,
                                                    SubmissionDetailFileSerilaizer, SubmissionListSerializer,
                                                    SubmissionLockSerializer, TeamSubmissionListSerializer)
            ...
            if validate:
                try:
                    ...
                    raise JsonHandlerError(
                        500, "Internal error getting submission info"
                    ) from exc
...
class StaffGraderMixin:
    ...
    def check_submission_lock(
        self, submission_uuid, data, suffix=""
        ...
        submission_lock = (
            SubmissionGradingLock.get_submission_lock(submission_uuid) or {}
        )
    ...
    @XBlock.json_handler
    @require_course_staff("STUDENT_GRADE")
    def batch_delete_submission_lock(
        self, data, suffix=""
    ):  # pylint: disable=unused-argument
        """
    ...
    def list_staff_workflows(self, data, suffix=""):  # pylint: disable=unused-argument
        ...
        for staff_workflow in staff_workflows:
            try:
                serialized_workflow = serializer(
                    staff_workflow, context=serializer_context
                ).data
                result[staff_workflow.identifying_uuid] = serialized_workflow
            except MissingContextException as e:
                log.exception(
                    "Failed to serialize workflow %d: %s",
                    staff_workflow.id,
                    str(e),
                    exc_info=True,
                )
    ...
    def _get_list_workflows_serializer_context(
        self, staff_workflows, is_team_assignment=False
    ):
        """
        ...
        if is_team_assignment:
            ...
            team_submission_uuid_to_team_id = get_team_ids_by_team_submission_uuid(
                submission_uuids
            )

            # Look up names for teams
            topic_id = self.selected_teamset_id
            team_id_to_team_name = self.teams_service.get_team_names(
                course_id, topic_id
            )
        ...
        submission_uuid_to_assessment = self.bulk_deep_fetch_assessments(
            staff_workflows
        )
    ...
    def _bulk_fetch_annotated_staff_workflows(self, is_team_assignment=False):
        ...
        newest_lock = (
            SubmissionGradingLock.currently_active()
            .filter(submission_uuid=OuterRef(identifying_uuid))
            .order_by("-created_at")
        )

        staff_workflows = (
            workflow_type.objects.filter(
            ...
            .annotate(
                grading_status=Case(
                    When(
                        ...
                        then=Value("graded", output_field=CharField()),
                    ),
    ...
    def bulk_deep_fetch_assessments(self, staff_workflows):
        ...
        for workflow in staff_workflows:
            if workflow.assessment:
                assessment_id_to_identifying_uuid[
                    workflow.assessment
                ] = workflow.identifying_uuid

        assessments = (
            Assessment.objects.filter(pk__in=assessment_id_to_identifying_uuid.keys())
            .prefetch_related(
                Prefetch(
                    "parts",
                    queryset=AssessmentPart.objects.select_related(
                        "criterion", "option"
                    ),
                ),
                "rubric__criteria",
    ...
    def submit_staff_assessment(
        ...
        """
        if self.is_team_assignment():
            success, err_msg = self.do_team_staff_assessment(
                data, team_submission_uuid=submission_uuid
            )
        else:
